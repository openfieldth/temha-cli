...
## 플러그인 & 애니메이션 (resources-common)
템하 프로젝트는 저장소 [`resources-common/`](../../resources-common/) 의 번들 JS/CSS를 모든 페이지에 공통 로드합니다.  
`gulp preview`·빌드 시 `프로젝트/resources/js|css/` 로 복사되며, 파일명은 **push 제외** 시스템 리소스입니다 (README 참고).

### 번들 구성

| 파일 | 라이브러리 | 버전(주석 기준) | 비고 |
|------|-----------|----------------|------|
| `plugin.js` | jQuery | 3.6.3 | `$`, `jQuery` |
| | Swiper | 11.1.14 | `Swiper`, `new Swiper('.swiper', { ... })` |
| | GSAP | 3.12.5 | `gsap` |
| | ScrollTrigger | 3.12.5 | `gsap.registerPlugin(ScrollTrigger)` 후 사용 |
| | TextPlugin | 3.12.5 | 텍스트 트윈 |
| | ScrollToPlugin | 3.12.5 | `gsap.to(window, { scrollTo: '#target' })` 등 |
| `plugin.css` | Swiper 스타일 | 11.1.14 | 마크업에 `.swiper` 구조 필요 |
| `setting.js` | AOS | 2.x 계열 | 파일 하단 `AOS.init()` 기본 실행 |
| `setting.css` | AOS 스타일 | — | `fade-up`, `fade-down` 등 |

**포함되지 않는 라이브러리**: Lenis, Locomotive Scroll, Mo.js, SplitType 등은 `plugin.js`에 **없습니다**.
...
<script src="./resources/js/temhagnu.js"></script>
<script src="./resources/js/templatehouse.js"></script>
<script src="./{페이지명}.style.js"></script>  <!-- 블록별 .js 합본, 있는 경우 -->
```

`setting.css`, `plugin.css`는 `<head>`에서 `templatehouse.css` 등과 함께 링크됩니다.

### AOS — 스크롤·로드 애니메이션 (권장)

`setting.js`에서 이미 `AOS.init()`가 호출됩니다. 블록 HTML에 `data-aos`만 추가하면 됩니다.

```html
<p data-aos="fade-up" data-aos-duration="1200" data-aos-easing="ease-out">라벨</p>
<h1 data-aos="fade-up" data-aos-duration="1200" data-aos-delay="200">제목</h1>
```

| 속성 | 설명 | 예시 |
|------|------|------|
| `data-aos` | 효과 이름 | `fade-up`, `fade-down`, `zoom-in` |
| `data-aos-duration` | 지속(ms) | `1200` (기본 400) |
| `data-aos-delay` | 지연(ms) | `0`, `200`, `400` |
| `data-aos-easing` | 이징 | `ease-out` |
| `data-aos-offset` | 트리거 오프셋(px) | 히어로 첫 화면은 `0` |
| `data-aos-once` | 1회만 재생 | `true` |

페이지별 옵션 재설정은 `{페이지}.style.js`에서 `AOS.init({ ... })` 호출 (이미 `setting.js` 이후 로드됨).

동적 DOM(Vue 그누보드 목록 등) 추가 후:
...
스크롤 연동·순차 등장·핀 고정 등 AOS로 부족할 때 사용합니다.
...
1. **입장 애니메이션**: 커스텀 `@keyframes` + `opacity: 0` 초기값보다 **`data-aos` 우선**.
2. **슬라이더**: Swiper 마크업 + `plugin.css` 클래스 사용. 별도 CDN 로드 금지.
3. **폰트**: 블록 CSS에 `font-family` 지정하지 않음. 페이지/세팅·`templatehouse.css` 토큰(`--ff-ko1` 등)으로 통일.
4. **강조 텍스트**: 장식용 `<em>`·블록 단 `font-style: italic` 지양. 필요 시 `<strong>` 또는 `font-weight`만 사용.
5. **시스템 파일 수정**: `resources-common`은 CLI 저장소 원본만 변경. 프로젝트 `resources/`의 `plugin.js`·`setting.js`는 덮어쓰기되므로 직접 수정하지 않음.
...
> 아래는 Temhakit(템하키트) 디자인 시스템 스펙이다. 모든 컴포넌트는 이 가이드의 클래스명, 색상 변수, 타이포그래피 토큰을 정확히 사용해라. 폰트는 `--ff-ko1`(Noto Sans KR) 기본, 색상은 `--primary: #d91f29` / `--secondary: #333850`을 기준으로 하고, 컴포넌트 상위 카테고리(유틸리티/콘텐츠/폼/상호작용/네비게이션/레이아웃/피드백)를 유지하며 생성해라. 스크롤·로드 애니메이션은 `resources-common`의 **AOS**(`data-aos`)를 우선 사용하고, 슬라이더는 **Swiper**, 복잡한 모션은 **GSAP**를 사용해라 (`plugin.js`·`setting.js` 참고).
