...
| 전체 링크 타입 | 내부 페이지 `page`, 외부 URL `url`, 이메일 `email`, 연락처 `tel`, 첨부파일 `file`을 실제 href 용도와 일치시킴 |
| 링크 누락 금지 | `header-gnblink`, `header-sublink`, `fullmenu-gnblink`, `fullmenu-sublink`, `footer-*link`, `btnset` 링크는 실제 링크일 때 `data-link-type` 필수 |
