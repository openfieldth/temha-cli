...
## 작업별 Topic 강제 로드 및 Preflight

`read_temha_rules()` 기본 로드만으로 블록을 작성하지 않는다. 사용자 요청이나 블록 성격에 따라 아래 topic 문서를 먼저 추가 확인한 뒤 `create_block` / `update_block`을 실행한다.

| 작업 유형 | 필수 추가 topic |
|----------|----------------|
| 헤더, 푸터, GNB, 전체메뉴, 2뎁스 메뉴 | `header-footer` |
| 폼, 문의, 신청, 예약, 개인정보 동의 | `forms` |
| 배경 이미지, 배경 비디오, 오버레이 | `background` |
| 메인 비주얼, 슬라이드, 스와이퍼, 캐러셀 | `swiper` |
| 버튼, 아이콘, 카드, 탭, 아코디언, 모달 | `components` |

작업 전 체크:
- [ ] 해당 블록 타입에 맞는 topic을 읽었는가
- [ ] topic 문서의 필수 class를 HTML에 반영했는가
- [ ] topic 문서의 금지 구조를 사용하지 않았는가
- [ ] width/height를 CSS 인라인 스타일이 아니라 `container-*` class와 블록 CSS로 제어했는가
...
### 링크 타입(data-link-type) 규칙

에디터는 `<a>` 태그의 `data-link-type` 값으로 링크 종류를 판별한다. 실제 이동/다운로드/연락 링크는 `href`와 `data-link-type`을 항상 함께 작성한다.

| 링크 종류 | 필수 속성 | href 예시 | 용도 |
|----------|----------|-----------|------|
| 페이지 | `data-link-type="page"` | `./greeting.html` | 템하 내부 페이지 이동 |
| URL | `data-link-type="url"` | `https://example.com` | 외부 사이트/일반 URL |
| 이메일 | `data-link-type="email"` | `mailto:help@example.com` | 이메일 연결 |
| 연락처 | `data-link-type="tel"` | `tel:01012345678` | 전화 연결 |
| 첨부파일 | `data-link-type="file"` | `/path/file.pdf` | 파일 다운로드 |

필수:
- 내부 페이지 이동은 반드시 `data-link-type="page"`를 사용한다.
- 외부 URL은 `data-link-type="url"`을 사용한다.
- 이메일/전화/첨부파일은 각각 `email` / `tel` / `file`을 사용한다.
- 헤더, 푸터, 버튼, 카드, 아이콘 링크 등 모든 실제 `<a>` 링크에 적용한다.
- 페이지 링크는 가능하면 `./page-name.html` 형식으로 작성한다.

금지:
- 내부 페이지 링크를 `data-link-type="url"`로 작성하지 않는다.
- `href`만 작성하고 `data-link-type`을 누락하지 않는다.
- 실제 링크 종류와 다른 타입(`tel`, `email` 등)을 임의로 넣지 않는다.
...
템플릿하우스는 `plugin.css` / `plugin.js`를 통해 아래 라이브러리를 기본 탑재한다. 별도 CDN `<script>` / `<link>`를 블록 HTML에 추가하지 않는다.

| 라이브러리 | 사용 목적 | 작성 규칙 |
|------------|-----------|-----------|
| `jQuery` | DOM 탐색, 이벤트, 블록 스코프 처리 | `$(function(){ ... })` + `$(".블록명[id='bid']")` 패턴 |
| `Swiper` | 메인 비주얼, 롤링, 캐러셀 | `topic: swiper` 구조 준수, 블록당 1개 권장 |
| `GSAP` | 등장 모션, 패럴랙스, 고급 인터랙션 | `window.gsap` 존재 확인 후 사용 |
| `ScrollTrigger` | 스크롤 기반 모션 | `window.ScrollTrigger` 확인 후 `gsap.registerPlugin(ScrollTrigger)` |
| `TextPlugin` | 텍스트 타이핑/교체 모션 | 필요 시 `window.TextPlugin` 확인 후 등록 |
| `ScrollToPlugin` | 섹션 이동/부드러운 스크롤 | 필요 시 `window.ScrollToPlugin` 확인 후 등록 |

주의:
- 외부 라이브러리 CDN을 중복 삽입하지 않는다.
- GSAP 계열은 존재 여부를 확인해 정적 화면도 깨지지 않게 작성한다.
- `Videoset`, `accordset`처럼 시스템이 자동 처리하는 컴포넌트에는 불필요한 JS를 작성하지 않는다.
## JavaScript 규칙
### 필수 패턴
...
| **contents-container** | `fullscreen` 클래스가 없다면 `.block-name .contents-container:not(.fullscreen) { height: ...; }` 패턴으로 높이값 설정 필수 |
...
### Header 필수 구조 Preflight
헤더 블록은 `read_temha_rules({ topic: "header-footer" })`를 먼저 읽고, 아래 class가 누락되면 생성/수정하지 않는다.

필수 구조:
- `header-container`
- `header-left`, `header-title`
- `header-center`, `header-gnb`, `header-gnblist`, `header-gnbitem`, `header-gnblink`
- 2뎁스가 있으면 `header-sublist`, `header-subitem`, `header-sublink`
- `header-right`, `header-utils`
- 유틸 버튼 항목은 `li.button`
- 전체 메뉴 항목은 `li.allmenu`, `btn-allmenu`, `ico-hamburger` 내부 `span` 3개
- 모바일 메뉴 버튼은 `btn-momenu`, `ico-hamburger` 내부 `span` 3개
- 전체 메뉴 레이어는 `header-fullmenu fullmenu-top|right|left|bottom`, `fullmenu-wrapper`, `fullmenu-head`, `fullmenu-title`, `fullmenu-gnblist`, `fullmenu-gnbitem`, `fullmenu-gnblink`, `fullmenu-close`
- 전체 메뉴에 2뎁스가 있으면 `fullmenu-sublist`, `fullmenu-subitem`, `fullmenu-sublink`

헤더 체크:
- [ ] 모든 1뎁스 `<a>`에 `header-gnblink`가 들어갔는가
- [ ] 2뎁스 메뉴는 hover/focus 시 보이도록 CSS가 작성되었는가
- [ ] `header-title`, `header-utils`, `btn-allmenu`, `btn-momenu`가 누락되지 않았는가
- [ ] 고정 CSS/JS를 `header-footer` topic 기준으로 포함했는가
### 푸터 블록
| 항목 | 규칙 |
|------|------|
| **CSS** | 고정 CSS 코드 전체 포함 필수 - `read_temha_rules({ topic: "header-footer" })` |
...
### 컨테이너 세로확장(100vh) 제어 Guard
에디터의 세로확장(100vh) 옵션은 `contents-container`에 `fullscreen` class를 추가/제거하는 방식으로 동작한다.

필수:
- 풀페이지/전체화면 비주얼은 `contents-container container-full fullscreen` 조합을 사용한다.
- 세로확장 OFF 상태의 기본 높이는 블록 CSS에서 `.block-name .contents-container:not(.fullscreen)`로만 제어한다.
- 히어로, 프로모션, 메인 비주얼처럼 고정 높이가 필요한 블록은 아래 패턴을 사용한다.

```css
.block-name .contents-container:not(.fullscreen) {
  height: 76rem;
}
...
금지:
- `contents-inner`에 `height`, `min-height`, `100vh`를 박아서 세로 확장을 제어하지 않는다.
- `.contents-container { min-height: 100vh; }`처럼 fullscreen 여부와 무관하게 100vh를 강제하지 않는다.
- 최상위 블록 class나 인라인 style로 세로확장 옵션을 대체하지 않는다.

### 컨테이너 가로폭 제어 Guard
에디터의 가로폭 옵션은 `contents-container`의 `container-lg`, `container-md`, `container-sm`, `container-full` class로 제어한다.

금지:
- `contents-inner`에 `width`, `max-width`, `min-width`를 박아서 1440px/1280px/1024px을 직접 제어하지 않는다.
- HTML `style="width:..."`, `style="max-width:..."`, `style="min-height:..."`로 컨테이너 크기를 제어하지 않는다.

허용:
- 1440px 기준 콘텐츠: `<div class="contents-container container-lg">`
- 1280px 기준 콘텐츠: `<div class="contents-container container-md">`
- 1024px 기준 콘텐츠: `<div class="contents-container container-sm">`
- 전체 폭/풀 비주얼: `<div class="contents-container container-full">`
- 세로 고정 높이는 `.block-name .contents-container:not(.fullscreen) { height: 76rem; }`처럼 `:not(.fullscreen)`에만 둔다.
