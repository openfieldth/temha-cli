...
async function validateToken() {
  try {
    ...
    const response = await get("https://temha.io/api/members/member/basic/me.json");
    if (response.data && (response.data.member || response.data.data?.member)) {
      return { valid: true };
    } else {
...
async function getAttachedFiles(projectId, cookiesArray) {
  const url2 = `https://temha.io/api/attach/file/list.json?use=projectMedia&useId=${projectId}`;
  const options = {
    withCredentials: true,
    headers: {
      "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36",
...
function replaceLocalResourceToApi(content, projectId, isCSS = false) {
  ...
  if (!isCSS) {
    content = content.replace(
      ...
      (match, attr, prefix, filename) => `${attr}="/api/dn/${projectId}/${filename}"`
    );
    content = content.replace(
      /srcset=["']([^"']+)["']/g,
      (match, srcsetValue) => {
        const processedSrcset = srcsetValue.split(",").map((srcItem) => {
          return srcItem.trim().replace(
            ...
            (urlMatch, prefix, filename) => `/api/dn/${projectId}/${filename}`
          );
        }).join(", ");
  ...
  content = content.replace(
    ...
    (match, prefix, filename) => `url("/api/dn/${projectId}/${filename}")`
  );
  return content;
}
...
async function uploadProjectResources(projectDir, projectId, cookies, cookiesArray) {
  ...
  try {
    const res = await getAttachedFiles(projectId, cookiesArray);
  ...
  const processDir = async (dir, type) => {
    ...
    for (const fileName of files) {
      ...
      try {
        ...
        const isExistingFile = remoteFilesMap.has(fileName);
        const existingFileKey = isExistingFile ? remoteFilesMap.get(fileName) : null;
        let fileStream = import_fs5.default.createReadStream(filePath);
        let customUrl = `/${projectId}/${fileName}`;
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
            if (attempt > 1) {
              ...
              fileStream = import_fs5.default.createReadStream(filePath);
            }
        ...
        if (!uploadSuccess) {
          if (isExistingFile && existingFileKey) {
            overwriteFailCount++;
            overwriteFailFiles.push({ file: fileName, error: lastError ? lastError.message : "" });
          } else {
            uploadFailCount++;
            uploadFailFiles.push({ file: fileName, error: lastError ? lastError.message : "" });
          }
        }
      } catch (error) {
        uploadFailCount++;
        uploadFailFiles.push({ file: fileName, error: error.message });
      }
    }
...
\uD398\uC774\uC9C0 \uCC98\uB9AC \uC2DC\uC791: ${pageFolderName}`);
        ...
        const sectionSummary = Object.entries(blocksBySection).filter(([k, v]) => v.length > 0).map(([k, v]) => `${k}: ${v.length}\uAC1C \uBE14\uB85D`).join(", ");
        if (sectionSummary) console.log("   - " + sectionSummary);
        if (!pageId) {
