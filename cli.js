...
async function uploadFile(fileStream, type, projectId, cookies, fileUrl = null, fileKey = null, fileName = null) {
  ...
  form.append("useId", projectId);
  if (fileUrl) {
    const processedUrl = fileUrl.replace(/[가-힣]/g, (char) => {
...
function askPassword(question) {
  ...
  if (!stdin.isTTY) {
    console.warn("\u26A0\uFE0F \uC774 \uD658\uACBD\uC5D0\uC11C\uB294 \uBE44\uBC00\uBC88\uD638 \uC785\uB825\uC774 \uD654\uBA74\uC5D0 \uD45C\uC2DC\uB420 \uC218 \uC788\uC2B5\uB2C8\uB2E4.");
    return ask(question);
  }
...
// src/utils/project-select.js
var import_fs4 = __toESM(require("fs"), 1);
var import_path4 = __toESM(require("path"), 1);
var import_readline2 = require("readline");
init_axios2();
function shouldOfferProjectChoiceMenu() {
  return Boolean(process.stdin.isTTY && process.stdout.isTTY);
}
async function fetchProjectsForPull(session) {
  const cookieHeader = Array.isArray(session.cookies) ? session.cookies.join("; ") : "";
  ...
  return projectList.filter((p) => !p.trash && (myOwnerIds.includes(p.id) || myEditorIds.includes(p.id))).map((p) => ({
    id: p.id,
    title: String(p.title ?? p.id ?? "(\uC774\uB984 \uC5C6\uC74C)")
  }));
}
...
async function promptPullProjectChoice(projects) {
  console.log("\n\uC544\uB798 \uD504\uB85C\uC81D\uD2B8\uB97C \uC120\uD0DD\uD574\uC8FC\uC138\uC694.");
  console.log(" 1. \uC804\uCCB4 \uD504\uB85C\uC81D\uD2B8 \uBD88\uB7EC\uC624\uAE30");
  projects.forEach((p, i) => {
    console.log(` ${i + 2}. ${p.title}(${p.id})`);
  });
  const raw = await askLine("\n\uC120\uD0DD \uBC88\uD638 \uC785\uB825 : ");
  const n = parseInt(raw, 10);
  if (raw === "" || Number.isNaN(n)) return null;
  if (n === 1) return "all";
  if (n >= 2 && n <= projects.length + 1) return projects[n - 2].id;
  return null;
}
function listLocalProjectsForPush(workspaceDir, metaFileName = "project.json") {
  if (!import_fs4.default.existsSync(workspaceDir)) return [];
  return import_fs4.default.readdirSync(workspaceDir).filter((name) => {
    const dirPath = import_path4.default.join(workspaceDir, name);
    return import_fs4.default.lstatSync(dirPath).isDirectory() && import_fs4.default.existsSync(import_path4.default.join(dirPath, metaFileName));
  }).map((folderName) => {
    const match = folderName.match(/\(([^)]+)\)$/);
    const id = match ? match[1] : null;
    const titleMatch = folderName.match(/^(.+?)\s*\([^)]+\)\s*$/);
    const title = titleMatch ? titleMatch[1].trim() : folderName.trim();
    return { folderName, id, title: title || folderName };
  }).filter((p) => p.id);
}
async function promptPushProjectChoice(localProjects) {
  console.log("\n\uC544\uB798 \uD504\uB85C\uC81D\uD2B8\uB97C \uC120\uD0DD\uD574\uC8FC\uC138\uC694.");
  console.log(" 1. \uC804\uCCB4 \uD504\uB85C\uC81D\uD2B8 \uC62C\uB9AC\uAE30");
  ...
  if (raw === "" || Number.isNaN(n)) return null;
  if (n === 1) return "all";
  if (n >= 2 && n <= localProjects.length + 1) return localProjects[n - 2].id;
  return null;
}
...
function logWarning(message, emoji = "\u26A0\uFE0F") {
  console.log(import_chalk.default.yellow(`${emoji} ${message}`));
}
...
function removeDirRecursive(targetPath) {
  if (import_fs7.default.existsSync(targetPath)) {
    import_fs7.default.readdirSync(targetPath).forEach((file) => {
      const curPath = import_path7.default.join(targetPath, file);
      if (import_fs7.default.lstatSync(curPath).isDirectory()) {
        removeDirRecursive(curPath);
      } else {
        import_fs7.default.unlinkSync(curPath);
      }
    });
    import_fs7.default.rmdirSync(targetPath);
  }
}
...
function fixCssUrlQuotes(str) {
  return str.replace(/url\((?!['"])([^)]+?)(["'])?\)/g, (match, path9, endQuote) => {
    return `url("${path9.replace(/^['"]|['"]$/g, "")}")`;
  });
}
function findResourceSubfolder(resourceDir, filename) {
  const candidates = ["icons", "images", "video", "css", "js"];
  for (const folder of candidates) {
    const filePath = import_path7.default.join(resourceDir, folder, filename);
...
async function downloadAndReplaceResources(str, resourceDir, downloadedSet, cookies = []) {
  ...
  while ((match = urlRegex.exec(str)) !== null) {
    ...
    const saveDir = import_path7.default.join(resourceDir, subfolder);
    if (!import_fs7.default.existsSync(saveDir)) import_fs7.default.mkdirSync(saveDir, { recursive: true });
    const savePath = import_path7.default.join(saveDir, fileName);
        ...
        downloads.push(
          axios_default.post("https://temha.io/api/attach/file/upload.json", form, {
            ...
            if (isNotFoundScript(res.data)) {
              ...
              import_fs7.default.writeFileSync(savePath, res.data);
        ...
        downloads.push(
          axios_default.get(downloadUrl, requestConfig).then((res) => {
            if (isNotFoundScript(res.data)) {
              ...
              import_fs7.default.writeFileSync(savePath, res.data, "utf8");
              if (!import_fs7.default.existsSync(savePath)) {
...
function migrateMetaJson(dir, type) {
  const oldPath = import_path7.default.join(dir, "meta.json");
  let newPath;
  if (type === "project") newPath = import_path7.default.join(dir, PROJECT_META_FILE);
  else if (type === "page") newPath = import_path7.default.join(dir, PAGE_META_FILE);
  else if (type === "block") newPath = import_path7.default.join(dir, BLOCK_META_FILE);
  else return;
  if (import_fs7.default.existsSync(oldPath) && !import_fs7.default.existsSync(newPath)) {
    import_fs7.default.renameSync(oldPath, newPath);
    console.log(`[\uB9C8\uC774\uADF8\uB808\uC774\uC158] meta.json \u2192 ${import_path7.default.basename(newPath)}`);
  }
}
...
function checkLocalChanges(workspaceDir) {
  if (!import_fs7.default.existsSync(workspaceDir)) {
    return { hasChanges: false, changes: [] };
  }
  const changes = [];
  try {
    const projectDirs = import_fs7.default.readdirSync(workspaceDir).filter((name) => {
      const dirPath = import_path7.default.join(workspaceDir, name);
      return import_fs7.default.lstatSync(dirPath).isDirectory() && import_fs7.default.existsSync(import_path7.default.join(dirPath, "project.json"));
    });
    for (const projectDir of projectDirs) {
      const projectPath = import_path7.default.join(workspaceDir, projectDir);
      const stats = import_fs7.default.statSync(projectPath);
...
async function saveHiddenProjectId(projectDir, projectId) {
  try {
    const hiddenPath = import_path7.default.join(projectDir, ".project_id");
    const encryptedId = encryptProjectId(projectId);
    import_fs7.default.writeFileSync(hiddenPath, encryptedId);
    if (process.platform === "win32") {
...
function hideProjectIdInVSCode() {
  try {
    const vscodeDir = import_path7.default.join(process.cwd(), ".vscode");
    const vscodeSettingsPath = import_path7.default.join(vscodeDir, "settings.json");
    const excludeConfig = { "files.exclude": { "**/.project_id": true } };
    if (!import_fs7.default.existsSync(vscodeDir)) {
      import_fs7.default.mkdirSync(vscodeDir);
    }
    let settings = {};
    if (import_fs7.default.existsSync(vscodeSettingsPath)) {
      try {
        settings = JSON.parse(import_fs7.default.readFileSync(vscodeSettingsPath, "utf8"));
      } catch {
      }
    }
    settings["files.exclude"] = Object.assign({}, settings["files.exclude"], excludeConfig["files.exclude"]);
    import_fs7.default.writeFileSync(vscodeSettingsPath, JSON.stringify(settings, null, 2));
  } catch (e) {
    console.warn("VSCode \uC228\uAE40 \uC124\uC815 \uC2E4\uD328:", e.message);
  }
}
async function pull({ projectId: initialProjectId, skipConfirmation = false }) {
  ...
  try {
    ...
    const sessionPath = import_path7.default.join(process.cwd(), ".temha_session.json");
    ...
    const session = JSON.parse(import_fs7.default.readFileSync(sessionPath, "utf8"));
    ...
    let workspaceDir = import_path7.default.join(process.cwd(), `${userName}-workspace`);
    try {
      const root2 = process.cwd();
      const allItems = import_fs7.default.readdirSync(root2);
      const oldWorkspaceDirs = allItems.filter((item) => {
        const itemPath = import_path7.default.join(root2, item);
        return import_fs7.default.lstatSync(itemPath).isDirectory() && item.endsWith("-workspace") && item !== `${userName}-workspace`;
      });
      if (oldWorkspaceDirs.length > 0) {
        console.log("\n\u{1F9F9} \uC774\uC804 workspace \uD3F4\uB354\uB4E4\uC744 \uC815\uB9AC\uD569\uB2C8\uB2E4...");
        for (const oldDir of oldWorkspaceDirs) {
          const oldDirPath = import_path7.default.join(root2, oldDir);
    ...
    if (!skipConfirmation && !projectId && shouldOfferProjectChoiceMenu()) {
      try {
        logInfo("\uC11C\uBC84\uC5D0\uC11C \uD504\uB85C\uC81D\uD2B8 \uBAA9\uB85D\uC744 \uAC00\uC838\uC624\uB294 \uC911...", "\u{1F4CB}");
        const projects = await fetchProjectsForPull(session);
        if (projects.length === 0) {
          logWarning("\uBD88\uB7EC\uC62C \uC218 \uC788\uB294 \uD504\uB85C\uC81D\uD2B8\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4. (owner/editor \uAD8C\uD55C \uB610\uB294 \uD734\uC9C0\uD1B5 \uC0C1\uD0DC\uB97C \uD655\uC778\uD574 \uC8FC\uC138\uC694.)", "\u2139\uFE0F");
          return;
        }
        const choice = await promptPullProjectChoice(projects);
        if (choice === null) {
          logWarning("\uC720\uD6A8\uD558\uC9C0 \uC54A\uC740 \uC120\uD0DD\uC785\uB2C8\uB2E4.", "\u26A0\uFE0F");
          return;
        }
        if (choice !== "all") {
          projectId = choice;
        ...
        logError(`\uD504\uB85C\uC81D\uD2B8 \uBAA9\uB85D\uC744 \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4: ${e.message}`, "\u274C");
        return;
      }
    }
    if (!skipConfirmation) {
...
\uC804\uCCB4 workspace \uD3F4\uB354\uAC00 \uC0C8\uB86D\uAC8C \uC0DD\uC131\uB418\uC5B4 \uC11C\uBC84\uC5D0\uC11C \uB2E4\uC6B4\uB85C\uB4DC\uB429\uB2C8\uB2E4.`);
    ...
    if (!projectId && import_fs7.default.existsSync(workspaceDir)) {
      console.log("\n\u{1F5D1}\uFE0F  \uAE30\uC874 workspace \uD3F4\uB354 \uC804\uCCB4 \uC0AD\uC81C \uC911...");
      removeDirRecursive(workspaceDir);
      console.log("\u2705 workspace \uD3F4\uB354 \uC0AD\uC81C \uC644\uB8CC");
    }
    if (projectId && import_fs7.default.existsSync(workspaceDir) && !skipConfirmation) {
      const allDirs = import_fs7.default.readdirSync(workspaceDir).filter((name) => {
        const dirPath = import_path7.default.join(workspaceDir, name);
        return import_fs7.default.lstatSync(dirPath).isDirectory();
      });
      for (const dir of allDirs) {
        const extractedProjectId = extractProjectIdFromFolderName2(dir);
        if (extractedProjectId === projectId) {
          console.log(`
\u{1F5D1}\uFE0F  \uAE30\uC874 \uD504\uB85C\uC81D\uD2B8 \uD3F4\uB354 \uC0AD\uC81C: ${dir}`);
          removeDirRecursive(import_path7.default.join(workspaceDir, dir));
        }
      }
    }
    if (!import_fs7.default.existsSync(workspaceDir)) {
      import_fs7.default.mkdirSync(workspaceDir, { recursive: true });
    }
    if (!projectId) {
      ...
      import_fs7.default.writeFileSync(sessionPath, JSON.stringify(session, null, 2), "utf8");
...
=== [${id}] \uD504\uB85C\uC81D\uD2B8 pull \uC2DC\uC791 ===`);
    ...
    try {
      ...
      if (memberRes.data?.data?.member) {
        ...
        const newWorkspaceDir = import_path7.default.join(process.cwd(), `${newUserName}-workspace`);
        if (newUserName !== userName) {
          console.log(`
\u{1F504} \uC720\uC800\uBA85\uC774 \uBCC0\uACBD\uB418\uC5C8\uC2B5\uB2C8\uB2E4: ${userName} \u2192 ${newUserName}`);
          if (import_fs7.default.existsSync(workspaceDir) && !import_fs7.default.existsSync(newWorkspaceDir)) {
            console.log(`\u{1F4C1} \uC6CC\uD06C\uC2A4\uD398\uC774\uC2A4 \uD3F4\uB354 \uC774\uB3D9: ${userName}-workspace \u2192 ${newUserName}-workspace`);
            import_fs7.default.renameSync(workspaceDir, newWorkspaceDir);
          }
    ...
    const projectDir = import_path7.default.join(workspaceDir, projectFolder);
    const rootDir = projectDir;
    removeDirRecursive(rootDir);
    import_fs7.default.mkdirSync(rootDir, { recursive: true });
    migrateMetaJson(rootDir, "project");
    const gnuboardDomain = extractGnuboardDomain(project);
    const metaData = {
      projectTitle: project.title,
      // projectId는 보안상 숨겨진 파일에 저장 (사용자 실수 방지)
      ...gnuboardDomain ? { gnuboard: { domain: gnuboardDomain } } : {}
    };
    import_fs7.default.writeFileSync(import_path7.default.join(rootDir, PROJECT_META_FILE), JSON.stringify(metaData, null, 2));
    ...
    try {
      ...
      if (sectionRes?.data?.section) {
        const session2 = JSON.parse(import_fs7.default.readFileSync(import_path7.default.join(process.cwd(), ".temha_session.json"), "utf8"));
        const sectionWithLogin = {
          ...sectionRes.data.section,
          member: session2.member
        };
        import_fs7.default.writeFileSync(import_path7.default.join(rootDir, "section.json"), JSON.stringify(sectionWithLogin, null, 2), "utf8");
      }
    } catch (e) {
      console.warn("section.json API fetch \uC2E4\uD328:", e.message);
    }
    const resourceDir = import_path7.default.join(rootDir, "resources");
    const cssDir = import_path7.default.join(resourceDir, "css");
    const jsDir = import_path7.default.join(resourceDir, "js");
    import_fs7.default.mkdirSync(cssDir, { recursive: true });
    import_fs7.default.mkdirSync(jsDir, { recursive: true });
    ...
    if (Array.isArray(resource.css)) {
      for (const cssObj of resource.css) {
        ...
        if (cssObj.fileCode) {
          import_fs7.default.writeFileSync(import_path7.default.join(cssDir, cssFileName), toLF(cssObj.fileCode));
          projectCssFiles++;
        }
      }
    }
    if (Array.isArray(resource.js)) {
      for (const jsObj of resource.js) {
        ...
        if (jsObj.fileCode) {
          import_fs7.default.writeFileSync(import_path7.default.join(jsDir, jsFileName), toLF(jsObj.fileCode));
          projectJsFiles++;
        }
      }
    }
    ...
    for (const { key, folder } of resourceTypes) {
      if (Array.isArray(resource[key])) {
        const saveDir = import_path7.default.join(resourceDir, folder);
        if (!import_fs7.default.existsSync(saveDir)) import_fs7.default.mkdirSync(saveDir, { recursive: true });
          ...
          if (!downloadedSet.has(downloadKey)) {
            try {
              ...
              if (isNotFoundScript(res.data)) {
                ...
                import_fs7.default.writeFileSync(import_path7.default.join(saveDir, fileName), res.data);
                downloadedSet.add(downloadKey);
              ...
              console.error(`[\uB2E4\uC6B4\uB85C\uB4DC \uC2E4\uD328] ${url2} \u2192 ${import_path7.default.join(saveDir, fileName)} : ${e.message}`);
            }
          }
        }
      }
            ...
            if (mediaList.length > 0) {
              ...
              for (const media of mediaList) {
                ...
                try {
                  ...
                  const filePath = import_path7.default.join(saveDir, fileName);
                  if (isNotFoundScript(res.data)) {
                    console.warn(`[\uC11C\uBC84\uC5D0 \uC5C6\uB294 \uD30C\uC77C] ${fileName} \uB2E4\uC6B4\uB85C\uB4DC \uC2A4\uD0B5`);
                  } else {
                    import_fs7.default.writeFileSync(filePath, res.data);
                    downloadedSet.add(downloadKey);
                    console.log(`    \u2705 ${fileName} \uB2E4\uC6B4\uB85C\uB4DC \uC644\uB8CC`);
    ...
    for (const page of pages.data.data.pageList) {
      ...
      const pageDir = import_path7.default.join(rootDir, pageFolderName);
      removeDirRecursive(pageDir);
      import_fs7.default.mkdirSync(pageDir, { recursive: true });
      const sectionTypes = ["header", "content", "footer"];
      const pageBlockOrder = [];
      let fullHtmlOutput = "";
      sectionTypes.forEach((section) => {
        const sectionPath = import_path7.default.join(pageDir, section);
        import_fs7.default.mkdirSync(sectionPath, { recursive: true });
      });
      for (const section of sectionTypes) {
        ...
        for (const block of blocks) {
          ...
          const rawFolderBase = block.classtitle && String(block.classtitle).trim() || block.name;
          const safeName = rawFolderBase.replace(/[<>:"/\\|?*]/g, "-");
          const blockDir = import_path7.default.join(pageDir, section, safeName);
          import_fs7.default.mkdirSync(blockDir, { recursive: true });
          let html = block.html || "";
          let css = block.css || "";
          ...
          const js = block.js || "";
          writeFileRecursive(import_path7.default.join(blockDir, `${safeName}.html`), html);
          writeFileRecursive(import_path7.default.join(blockDir, `${safeName}.css`), css);
          writeFileRecursive(import_path7.default.join(blockDir, `${safeName}.js`), js);
          const meta = {
            name: block.name,
            classtitle: block.classtitle && String(block.classtitle).trim() || block.name,
            // id: block.id,  // id 제거 - push 시 자동 생성
            bid: block.bid,
            // bid는 유지
            title: block.title || "",
            thumbnail: block.thumbnail || ""
          };
          writeFileRecursive(import_path7.default.join(blockDir, BLOCK_META_FILE), JSON.stringify(meta, null, 2));
          pageBlockOrder.push({
            block: safeName,
            section
            // pageId 제거 - 보안상 숨김처리
...
`;
      ...
      const blockOrderPath = import_path7.default.join(pageDir, "block_order.json");
      import_fs7.default.writeFileSync(blockOrderPath, JSON.stringify(pageBlockOrder, null, 2), "utf8");
      const cssFiles = import_fs7.default.existsSync(cssDir) ? import_fs7.default.readdirSync(cssDir).filter((f) => f.endsWith(".css")) : [];
      const jsFiles = import_fs7.default.existsSync(jsDir) ? import_fs7.default.readdirSync(jsDir).filter((f) => f.endsWith(".js")) : [];
      const headHtml = `
<!DOCTYPE html>
<html lang="ko">
<head>
...
${fullHtmlOutput.trim()}
      ...
      import_fs7.default.writeFileSync(import_path7.default.join(pageDir, PAGE_META_FILE), JSON.stringify(pageMeta, null, 2), "utf8");
    ...
    const pageOrderPath = import_path7.default.join(rootDir, "pageOrder.json");
    import_fs7.default.writeFileSync(pageOrderPath, JSON.stringify(pageOrderData, null, 2), "utf8");
...
var KoreanToRomanization = class {
  ...
  convertFileName(fileName) {
    ...
    const hasKorean = /[가-힣]/.test(nameWithoutSpaces);
    if (!hasKorean) {
...
function logWarning2(message, emoji = "\u26A0\uFE0F") {
...
function extractProjectIdFromFolderName3(folderName) {
  const match = folderName.match(/\(([^)]+)\)$/);
  return match ? match[1] : null;
}
...
function getExistingPageIds(projectDir) {
  ...
  try {
    const items = import_fs8.default.readdirSync(projectDir);
    for (const item of items) {
      const itemPath = import_path8.default.join(projectDir, item);
      if (import_fs8.default.lstatSync(itemPath).isDirectory() && import_fs8.default.existsSync(import_path8.default.join(itemPath, PAGE_META_FILE2))) {
...
function loadPageMeta(pageDir) {
  ...
  const metaPath = import_path8.default.join(pageDir, PAGE_META_FILE2);
  if (!import_fs8.default.existsSync(metaPath)) return null;
  try {
    let meta = JSON.parse(import_fs8.default.readFileSync(metaPath, "utf8"));
    let changed = false;
    if (!meta.pageName) {
      meta.pageName = import_path8.default.basename(pageDir) + ".html";
      changed = true;
...
function extractRootClassFromBlockHtml(blockPath) {
  if (!import_fs8.default.existsSync(blockPath)) return null;
  let htmlFiles;
  try {
    htmlFiles = import_fs8.default.readdirSync(blockPath).filter((f) => f.endsWith(".html"));
  } catch {
    return null;
  }
  for (const f of htmlFiles) {
    const html = readFileSyncUtf8(import_path8.default.join(blockPath, f));
    const m = html.match(/<section[^>]*\sclass=["']([^"']+)["']/i);
    if (m) {
      const first = m[1].trim().split(/\s+/).filter(Boolean)[0];
...
function detectAndAddMissingBlocks(pagePath, orderData) {
  ...
  const blockOrderPath = import_path8.default.join(pagePath, "block_order.json");
  ...
  sections.forEach((section) => {
    const sectionPath = import_path8.default.join(pagePath, section);
    if (!import_fs8.default.existsSync(sectionPath)) return;
    try {
      const blockFolders = import_fs8.default.readdirSync(sectionPath).filter((item) => {
        const itemPath = import_path8.default.join(sectionPath, item);
        return import_fs8.default.lstatSync(itemPath).isDirectory();
      });
      blockFolders.forEach((blockFolder) => {
        const blockKey = `${section}/${blockFolder}`;
        const blockPath = import_path8.default.join(sectionPath, blockFolder);
        const blockMetaPath = import_path8.default.join(blockPath, "block.json");
        if (import_fs8.default.existsSync(blockMetaPath)) {
      ...
      logWarning2(`\uC139\uC158 \uC2A4\uCE94 \uC2E4\uD328: ${section} - ${e.message}`, "\u26A0\uFE0F");
    }
  });
  const originalLength = orderData.length;
  orderData = orderData.filter((item) => {
    const sectionPath = import_path8.default.join(pagePath, item.section);
    const blockPath = import_path8.default.join(sectionPath, item.block);
    const blockMetaPath = import_path8.default.join(blockPath, "block.json");
    if (!import_fs8.default.existsSync(blockMetaPath)) {
      logWarning2(`\uC874\uC7AC\uD558\uC9C0 \uC54A\uB294 \uBE14\uB85D \uC81C\uAC70: ${item.section}/${item.block}`, "\u{1F5D1}\uFE0F");
      return false;
    }
    return true;
  });
  ...
  if (needsUpdate) {
    try {
      import_fs8.default.writeFileSync(blockOrderPath, JSON.stringify(orderData, null, 2));
      const messages = [];
...
function loadBlocksByOrder(pagePath, projectId) {
  ...
  const blockOrderPath = import_path8.default.join(pagePath, "block_order.json");
  if (!import_fs8.default.existsSync(blockOrderPath)) return null;
  ...
  orderData.forEach((blockInfo, index) => {
    ...
    const blockPath = import_path8.default.join(pagePath, section, blockFolderName);
    migrateMetaJson2(blockPath, "block");
    const metaPath = import_path8.default.join(blockPath, BLOCK_META_FILE2);
    ...
    if (usedIds.has(meta.id)) {
      ...
      meta.id = utilIdStamp();
      metaChanged = true;
      logWarning2(`\uC911\uBCF5 ID \uBC1C\uACAC! ${oldId} \u2192 ${meta.id} \uB85C \uBCC0\uACBD`, "\u26A0\uFE0F");
    }
    if (usedBids.has(meta.bid)) {
      const oldBid = meta.bid;
      meta.bid = utilBlockStamp();
      metaChanged = true;
      logWarning2(`\uC911\uBCF5 BID \uBC1C\uACAC! ${oldBid} \u2192 ${meta.bid} \uB85C \uBCC0\uACBD`, "\u26A0\uFE0F");
    }
    usedIds.add(meta.id);
    ...
    if (!meta.classtitle || typeof meta.classtitle !== "string" || meta.classtitle.trim() === "") {
      const fromHtml = extractRootClassFromBlockHtml(blockPath);
      meta.classtitle = fromHtml || blockFolderName;
      metaChanged = true;
    }
    let originalClass = meta.classtitle.trim();
    ...
    if (className !== originalClass) {
      meta.classtitle = className;
      metaChanged = true;
      logWarning2(`classtitle \uC911\uBCF5! ${originalClass} \u2192 ${className} \uB85C \uBCC0\uACBD`, "\u26A0\uFE0F");
    }
    if (sectionClassNames[section]) sectionClassNames[section].add(meta.classtitle);
    const extensions = [".html", ".css", ".js"];
    extensions.forEach((ext) => {
      const expectedFile = import_path8.default.join(blockPath, `${blockFolderName}${ext}`);
      const files = import_fs8.default.readdirSync(blockPath).filter((f) => f.endsWith(ext));
      files.forEach((file) => {
        const filePath = import_path8.default.join(blockPath, file);
        if (file !== `${blockFolderName}${ext}`) {
          try {
            import_fs8.default.renameSync(filePath, expectedFile);
            logInfo2(`\uD30C\uC77C\uBA85 \uBCC0\uACBD: ${file} \u2192 ${blockFolderName}${ext}`, "\u{1F4C1}");
          } catch (e) {
            logWarning2(`\uD30C\uC77C\uBA85 \uBCC0\uACBD \uC2E4\uD328: ${file} \u2192 ${blockFolderName}${ext} - ${e.message}`, "\u26A0\uFE0F");
          }
        }
      });
    });
    ...
    const blockName = blockFolderName;
    const findActualFile = (dir, extension) => {
      if (!import_fs8.default.existsSync(dir)) return null;
      const files = import_fs8.default.readdirSync(dir).filter((f) => f.endsWith(extension));
      if (files.length === 0) return null;
      const preferredFile = files.find((f) => f === `${blockFolderName}${extension}`);
    ...
    const htmlPath = htmlFile ? import_path8.default.join(blockPath, htmlFile) : null;
    const cssPath = cssFile ? import_path8.default.join(blockPath, cssFile) : null;
    const jsPath = jsFile ? import_path8.default.join(blockPath, jsFile) : null;
    let html = htmlPath && import_fs8.default.existsSync(htmlPath) ? readFileSyncUtf8(htmlPath) : "";
    let css = cssPath && import_fs8.default.existsSync(cssPath) ? readFileSyncUtf8(cssPath) : "";
    const js = jsPath && import_fs8.default.existsSync(jsPath) ? readFileSyncUtf8(jsPath) : "";
    const fileSummary = [
    ...
    if (!html.trim() && !css.trim() && !js.trim()) {
      console.log(import_chalk4.default.yellow(`[WARN] \uB0B4\uC6A9\uC774 \uBE44\uC5B4\uC788\uB294 \uBE14\uB85D(${meta.name}/${blockFolderName})\uC744 \uAC74\uB108\uB701\uB2C8\uB2E4.`));
      return;
    }
...
async function updatePageOrder(projectId, pageIdList, cookies, pageIdSort = {}) {
  ...
  if (res.data.code === "SUCC") {
    ...
    logWarning2(`\uD398\uC774\uC9C0 \uC21C\uC11C \uC815\uB82C \uC2E4\uD328: ${res.data?.msg || "\uC54C \uC218 \uC5C6\uB294 \uC624\uB958"}`, "\u26A0\uFE0F");
  }
}
function loadSession2() {
  const sessionPath = import_path8.default.join(process.cwd(), SESSION_FILE2);
  if (!import_fs8.default.existsSync(sessionPath)) {
    return null;
  }
  try {
    const session = JSON.parse(import_fs8.default.readFileSync(sessionPath, "utf8"));
    return session;
  } catch (e) {
    console.error("\uC138\uC158 \uD30C\uC77C \uC77D\uAE30 \uC2E4\uD328:", e.message);
    return null;
...
async function uploadProjectResources(projectDir, projectId, cookies, cookiesArray) {
  const resourceDir = import_path8.default.join(projectDir, "resources");
  if (!import_fs8.default.existsSync(resourceDir)) {
  ...
  const collectLocalFiles = (dir) => {
    ...
    files.forEach((fileName) => {
      ...
      if (!import_fs8.default.existsSync(filePath) || import_fs8.default.lstatSync(filePath).isDirectory())
        return;
      localFiles.add(fileName);
    });
  };
  const imagesDir = import_path8.default.join(resourceDir, "images");
  const iconsDir = import_path8.default.join(resourceDir, "icons");
  collectLocalFiles(imagesDir);
  collectLocalFiles(import_path8.default.join(resourceDir, "video"));
  collectLocalFiles(iconsDir);
  const iconFileNames = new Set(import_fs8.default.existsSync(iconsDir) ? import_fs8.default.readdirSync(iconsDir) : []);
  ...
  const processDir = async (dir, type) => {
    ...
    for (const fileName of files) {
      const filePath = import_path8.default.join(dir, fileName);
      if (!import_fs8.default.existsSync(filePath)) continue;
      if (import_fs8.default.lstatSync(filePath).isDirectory()) continue;
      if (type === "image" && iconFileNames.has(fileName)) {
        skippedCount++;
        continue;
      }
      const convertedFileName = korean_romanization_default.convertFileName(fileName);
      const hasKoreanFileName = /[가-힣]/.test(fileName);
      if (hasKoreanFileName) {
        logInfo2(`\uD55C\uAE00 \uD30C\uC77C\uBA85 \uBCC0\uD658: "${fileName}" \u2192 "${convertedFileName}"`, "\u{1F504}");
        const newFilePath = import_path8.default.join(dir, convertedFileName);
        try {
          import_fs8.default.renameSync(filePath, newFilePath);
      ...
      try {
        ...
        const finalFilePath = hasKoreanFileName ? import_path8.default.join(dir, convertedFileName) : filePath;
        let fileStream = import_fs8.default.createReadStream(finalFilePath);
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
            if (attempt > 1) {
              ...
              fileStream = import_fs8.default.createReadStream(finalFilePath);
            }
  ...
  await processDir(import_path8.default.join(resourceDir, "images"), "image");
  await processDir(import_path8.default.join(resourceDir, "video"), "video");
  await processDir(import_path8.default.join(resourceDir, "icons"), "icon");
  await updateProjectCSSJS(projectId, resourceDir, cookiesArray);
}
function migrateMetaJson2(dir, type) {
  const oldPath = import_path8.default.join(dir, "meta.json");
  let newPath;
  if (type === "project") newPath = import_path8.default.join(dir, PROJECT_META_FILE2);
  else if (type === "page") newPath = import_path8.default.join(dir, PAGE_META_FILE2);
  else if (type === "block") newPath = import_path8.default.join(dir, BLOCK_META_FILE2);
  else return;
  if (import_fs8.default.existsSync(oldPath) && !import_fs8.default.existsSync(newPath)) {
    import_fs8.default.renameSync(oldPath, newPath);
    console.log(`[\uB9C8\uC774\uADF8\uB808\uC774\uC158] meta.json \u2192 ${import_path8.default.basename(newPath)}`);
  }
}
async function push({ projectId: initialProjectId, deletePageId }) {
  let projectId = initialProjectId;
  const session = loadSession2();
  if (!session) {
  ...
  const workspaceDirName = import_fs8.default.readdirSync(process.cwd()).find(
    (dir) => dir.endsWith("-workspace") && import_fs8.default.lstatSync(import_path8.default.join(process.cwd(), dir)).isDirectory()
  );
  if (!workspaceDirName) {
    console.error(
  ...
  const root = import_path8.default.join(process.cwd(), workspaceDirName);
  if (!projectId && shouldOfferProjectChoiceMenu()) {
    const localProjects = listLocalProjectsForPush(root, PROJECT_META_FILE2);
    if (localProjects.length === 0) {
      console.error(
        "\uD478\uC2DC\uD560 \uB85C\uCEEC \uD504\uB85C\uC81D\uD2B8\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4. \uC6CC\uD06C\uC2A4\uD398\uC774\uC2A4\uC5D0 `project.json`\uC774 \uC788\uB294 \uD504\uB85C\uC81D\uD2B8 \uD3F4\uB354\uAC00 \uD544\uC694\uD569\uB2C8\uB2E4."
      );
      return;
    }
    const choice = await promptPushProjectChoice(localProjects);
    if (choice === null) {
      console.error("\uC720\uD6A8\uD558\uC9C0 \uC54A\uC740 \uC120\uD0DD\uC785\uB2C8\uB2E4.");
      return;
    }
    if (choice !== "all") {
      projectId = choice;
  ...
  let projectDirs = import_fs8.default.readdirSync(root).filter((name) => import_fs8.default.existsSync(import_path8.default.join(root, name, PROJECT_META_FILE2)));
  if (projectId) {
  ...
  let allProjectDirs = import_fs8.default.readdirSync(root).filter((name) => import_fs8.default.existsSync(import_path8.default.join(root, name, PROJECT_META_FILE2)));
  if (projectId) {
    const matched = allProjectDirs.find((name) => name.includes(`(${projectId})`));
    projectDirs = matched ? [matched] : allProjectDirs;
  } else {
  ...
  for (const projectName of projectDirs) {
    let projectDir = import_path8.default.join(root, projectName);
    let folderProjectId = projectId;
    if (!folderProjectId) {
      folderProjectId = extractProjectIdFromFolderName3(projectName);
      if (folderProjectId) {
    ...
    const projectMeta = readJsonSyncSafe(
      import_path8.default.join(projectDir, PROJECT_META_FILE2)
    );
          ...
          if (serverTitle !== localTitle) {
            ...
            if (updateResult && updateResult.code === "SUCC") {
              ...
              try {
                ...
                const newProjectDir = import_path8.default.join(root, newFolderName);
                if (projectDir !== newProjectDir) {
                  console.log(
                    `\uD3F4\uB354\uBA85 \uBCC0\uACBD: "${projectName}" \u2192 "${newFolderName}"`
                  );
                  if (import_fs8.default.existsSync(newProjectDir)) {
                    console.warn(
                      `\u26A0\uFE0F  \uD3F4\uB354\uBA85 \uBCC0\uACBD \uC2E4\uD328: "${newFolderName}" \uD3F4\uB354\uAC00 \uC774\uBBF8 \uC874\uC7AC\uD569\uB2C8\uB2E4.`
                    );
                  } else {
                    import_fs8.default.renameSync(projectDir, newProjectDir);
    ...
    while (restart) {
      restart = false;
      pageDirs = import_fs8.default.readdirSync(projectDir).filter(
        (name) => import_fs8.default.existsSync(import_path8.default.join(projectDir, name, PAGE_META_FILE2)) && import_fs8.default.existsSync(import_path8.default.join(projectDir, name, "block_order.json")) && !name.endsWith(".html")
      );
      const newlyGeneratedPageIds = /* @__PURE__ */ new Set();
      const pageIdToFolders = {};
      for (const pageFolderName of pageDirs) {
        const pageDir = import_path8.default.join(projectDir, pageFolderName);
      ...
      for (const [pageId, folders] of Object.entries(pageIdToFolders)) {
        if (folders.length > 1) {
          ...
          const folderInfos = folders.map((folderName) => {
            const dirPath = import_path8.default.join(projectDir, folderName);
            const stat = import_fs8.default.statSync(dirPath);
            return {
              folderName,
              dirPath,
              mtime: stat.mtimeMs,
          ...
          for (let i = 1; i < folderInfos.length; i++) {
            ...
            const newPageDir = import_path8.default.join(projectDir, newPageFolder);
            try {
              import_fs8.default.renameSync(oldDirPath, newPageDir);
      ...
      for (const pageFolderName of pageDirs) {
        let pageDir = import_path8.default.join(projectDir, pageFolderName);
        console.log(`
\uD398\uC774\uC9C0 \uCC98\uB9AC \uC2DC\uC791: ${pageFolderName}`);
        ...
        if (!pageMeta) {
          ...
          if (pageId2) {
            ...
            const expectedDir = import_path8.default.join(projectDir, expectedFolder);
            if (expectedDir !== pageDir && import_fs8.default.existsSync(expectedDir)) {
              restart = true;
              break;
            }
          }
        ...
        if (!pageId) {
          ...
          if (pageDir !== newPageDir) {
            import_fs8.default.renameSync(pageDir, newPageDir);
            console.log(
          ...
          if (pageFolderName !== expectedFolder) {
            const newPageDir = import_path8.default.join(projectDir, expectedFolder);
            import_fs8.default.renameSync(pageDir, newPageDir);
            console.log(
    ...
    const pageOrderPath = import_path8.default.join(projectDir, "pageOrder.json");
    let pageOrder = {};
    let hasPageOrderFile = false;
    if (import_fs8.default.existsSync(pageOrderPath)) {
      try {
        const pageOrderData = JSON.parse(import_fs8.default.readFileSync(pageOrderPath, "utf8"));
    ...
    if (pushedPageIds.length > 0) {
      const pageIdToTitle = {};
      const titleToPageId = {};
      const currentPageDirs = import_fs8.default.readdirSync(projectDir).filter(
        (name) => import_fs8.default.existsSync(import_path8.default.join(projectDir, name, PAGE_META_FILE2)) && import_fs8.default.existsSync(import_path8.default.join(projectDir, name, "block_order.json")) && !name.endsWith(".html")
      );
      ...
      import_fs8.default.writeFileSync(pageOrderPath, JSON.stringify(updatedPageOrder, null, 2), "utf8");
    }
    try {
      ...
      if (serverPagesRes?.data?.code === "SUCC") {
        ...
        logWarning2(`\uC11C\uBC84 \uD398\uC774\uC9C0 \uBAA9\uB85D \uC870\uD68C \uC2E4\uD328: ${serverPagesRes?.data?.msg}`, "\u26A0\uFE0F");
      }
    } catch (error) {
      logWarning2(`\uC11C\uBC84 \uD398\uC774\uC9C0 \uB3D9\uAE30\uD654 \uC911 \uC624\uB958: ${error.message}`, "\u26A0\uFE0F");
    }
  }
  console.log("\n\uC804\uCCB4 push \uC644\uB8CC");
  ...
  for (const projectName of projectDirs) {
    const projectDir = import_path8.default.join(root, projectName);
    const metaPath = import_path8.default.join(projectDir, PROJECT_META_FILE2);
    ...
    const pageDirs = import_fs8.default.readdirSync(projectDir).filter(
      (name) => import_fs8.default.existsSync(import_path8.default.join(projectDir, name, PAGE_META_FILE2))
    );
    for (const pageFolderName of pageDirs) {
      const pageDir = import_path8.default.join(projectDir, pageFolderName);
      const pageMetaPath = import_path8.default.join(pageDir, PAGE_META_FILE2);
      if (import_fs8.default.existsSync(pageMetaPath)) {
        try {
          const pageMeta = readJsonSyncSafe(pageMetaPath);
          if (pageMeta && pageMeta.pageId) {
            delete pageMeta.pageId;
            import_fs8.default.writeFileSync(pageMetaPath, JSON.stringify(pageMeta, null, 2));
          }
        } catch (e) {
        }
      }
  ...
  for (const projectName of projectDirs) {
    ...
    for (const pageFolderName of pageDirs) {
      const pageDir = import_path8.default.join(projectDir, pageFolderName);
      const blockOrderPath = import_path8.default.join(pageDir, "block_order.json");
      if (import_fs8.default.existsSync(blockOrderPath)) {
        try {
          const orderData = JSON.parse(import_fs8.default.readFileSync(blockOrderPath, "utf8"));
  ...
  for (const projectName of projectDirs) {
    ...
    if (import_fs8.default.existsSync(resourceDir)) {
      const imagesDir = import_path8.default.join(resourceDir, "images");
      const iconsDir = import_path8.default.join(resourceDir, "icons");
      const videoDir = import_path8.default.join(resourceDir, "video");
      if (import_fs8.default.existsSync(imagesDir)) {
        totalImages += import_fs8.default.readdirSync(imagesDir).filter((f) => !import_fs8.default.lstatSync(import_path8.default.join(imagesDir, f)).isDirectory()).length;
      }
      if (import_fs8.default.existsSync(iconsDir)) {
        totalIcons += import_fs8.default.readdirSync(iconsDir).filter((f) => !import_fs8.default.lstatSync(import_path8.default.join(iconsDir, f)).isDirectory()).length;
      }
      if (import_fs8.default.existsSync(videoDir)) {
        totalVideos += import_fs8.default.readdirSync(videoDir).filter((f) => !import_fs8.default.lstatSync(import_path8.default.join(videoDir, f)).isDirectory()).length;
      }
    }
  }
  const resourceStats = {
...
async function updateProjectCSSJS(projectId, resourceDir, cookiesArray) {
  const cssDir = import_path8.default.join(resourceDir, "css");
  const jsDir = import_path8.default.join(resourceDir, "js");
  const cssExists = import_fs8.default.existsSync(cssDir);
  const jsExists = import_fs8.default.existsSync(jsDir);
  const systemFiles = [
  ...
  try {
    ...
    const commonDir = import_path8.default.join(process.cwd(), "resources-common");
    const hasCommonDir = import_fs8.default.existsSync(commonDir);
    if (hasCommonDir) {
      ...
      for (const file of systemFiles) {
        ...
        if (!serverFiles.has(file.name)) {
          ...
          const commonFilePath = import_path8.default.join(commonDir, file.name);
          if (import_fs8.default.existsSync(commonFilePath)) {
            console.log(`\u{1F4E4} ${file.name} \uD30C\uC77C\uC744 resources-common\uC5D0\uC11C \uCD94\uAC00 \uC911...`);
            try {
              const fileContent = import_fs8.default.readFileSync(commonFilePath, "utf8");
              const fileName = file.name;
              const name = import_path8.default.basename(fileName, import_path8.default.extname(fileName));
              const fileItem = {
                type: "code",
                name,
                fileName,
    ...
    if (cssExists) {
      const cssFiles = import_fs8.default.readdirSync(cssDir).filter((f) => f.endsWith(".css"));
      logSection2(`[CSS] ${cssFiles.length}\uAC1C \uD30C\uC77C \uCC98\uB9AC`, "\u{1F3A8}");
      for (const cssFile of cssFiles) {
        const cssPath = import_path8.default.join(cssDir, cssFile);
        const cssContent = import_fs8.default.readFileSync(cssPath, "utf8");
        const cssName = import_path8.default.basename(cssFile, ".css");
        ...
        const cssItem = {
          ...
          fileEdit: false,
          // API 문서의 File 타입 요구사항에 맞는 추가 필드들
    ...
    const localCssNames = cssExists ? new Set(
      import_fs8.default.readdirSync(cssDir).filter((f) => f.endsWith(".css")).map((f) => import_path8.default.basename(f, ".css"))
    ) : /* @__PURE__ */ new Set();
    ...
    if (jsExists) {
      const jsFiles = import_fs8.default.readdirSync(jsDir).filter((f) => f.endsWith(".js"));
      logSection2(`[JS] ${jsFiles.length}\uAC1C \uD30C\uC77C \uCC98\uB9AC`, "\u26A1");
      for (const jsFile of jsFiles) {
        const jsPath = import_path8.default.join(jsDir, jsFile);
        const jsContent = import_fs8.default.readFileSync(jsPath, "utf8");
        const jsName = import_path8.default.basename(jsFile, ".js");
    ...
    const localJsNames = jsExists ? new Set(
      import_fs8.default.readdirSync(jsDir).filter((f) => f.endsWith(".js")).map((f) => import_path8.default.basename(f, ".js"))
    ) : /* @__PURE__ */ new Set();
...
async function recoverProjectId(projectDir, projectName, cookiesArray, newlyGeneratedIds = /* @__PURE__ */ new Set()) {
  ...
  const hiddenProjectId = readHiddenProjectId2(projectDir);
  if (hiddenProjectId) {
  ...
  try {
    const workspaceDir = import_path8.default.dirname(projectDir);
    const siblingDirs = import_fs8.default.readdirSync(workspaceDir).filter((name) => {
      const dirPath = import_path8.default.join(workspaceDir, name);
      return import_fs8.default.lstatSync(dirPath).isDirectory() && import_fs8.default.existsSync(import_path8.default.join(dirPath, PROJECT_META_FILE2)) && name !== import_path8.default.basename(projectDir);
    });
    for (const siblingDir of siblingDirs) {
      const extractedId = extractProjectIdFromFolderName3(siblingDir);
      if (extractedId) {
...
async function autoRecoverFolderName(projectDir, projectName, projectId, root) {
  try {
    ...
    if (projectDetails?.data?.project?.title) {
      ...
      const newProjectDir = import_path8.default.join(root, newFolderName);
      if (projectDir !== newProjectDir && !import_fs8.default.existsSync(newProjectDir)) {
        console.log(
...
function detectFolderNameChange(projectDir, projectId) {
  const currentFolderName = import_path8.default.basename(projectDir);
...
function encryptProjectId2(projectId) {
  try {
    const key = import_crypto5.default.scryptSync("temha-cli-secure-key-2024", "salt", 32);
    const iv = import_crypto5.default.randomBytes(16);
    const cipher = import_crypto5.default.createCipheriv("aes-256-cbc", key, iv);
    let encrypted = cipher.update(projectId, "utf8", "hex");
    encrypted += cipher.final("hex");
    return iv.toString("hex") + ":" + encrypted;
  } catch (e) {
...
function decryptProjectId(encryptedId) {
  try {
    ...
    const decipher = import_crypto5.default.createDecipheriv("aes-256-cbc", key, iv);
...
async function saveHiddenProjectId2(projectDir, projectId) {
  try {
    const hiddenPath = import_path8.default.join(projectDir, ".project_id");
    if (import_fs8.default.existsSync(hiddenPath)) {
      try {
        import_fs8.default.unlinkSync(hiddenPath);
      } catch (e) {
        console.warn("\uAE30\uC874 \uC228\uAE40\uD30C\uC77C \uC0AD\uC81C \uC2E4\uD328:", e.message);
      }
    }
    const encryptedId = encryptProjectId2(projectId);
    import_fs8.default.writeFileSync(hiddenPath, encryptedId);
    if (process.platform === "win32") {
      const { execSync } = await import("child_process");
      try {
        execSync(`attrib +h "${hiddenPath}"`, { stdio: "ignore" });
...
function readHiddenProjectId2(projectDir) {
  try {
    const hiddenPath = import_path8.default.join(projectDir, ".project_id");
    if (!import_fs8.default.existsSync(hiddenPath)) {
      return null;
    }
    const encryptedId = import_fs8.default.readFileSync(hiddenPath, "utf8").trim();
...
function hideProjectIdInVSCode2() {
  try {
    const vscodeDir = import_path8.default.join(process.cwd(), ".vscode");
    const vscodeSettingsPath = import_path8.default.join(vscodeDir, "settings.json");
    const excludeConfig = { "files.exclude": { "**/.project_id": true } };
    if (!import_fs8.default.existsSync(vscodeDir)) {
      import_fs8.default.mkdirSync(vscodeDir);
    }
    let settings = {};
    if (import_fs8.default.existsSync(vscodeSettingsPath)) {
      try {
        settings = JSON.parse(import_fs8.default.readFileSync(vscodeSettingsPath, "utf8"));
      } catch {
      }
    }
    ...
    import_fs8.default.writeFileSync(vscodeSettingsPath, JSON.stringify(settings, null, 2));
  } catch (e) {
    console.warn("VSCode \uC228\uAE40 \uC124\uC815 \uC2E4\uD328:", e.message);
  }
}
async function createNewProjectOnServer(projectDir, projectTitle, cookiesArray) {
  ...
  const project = {
    id: newId,
    title: projectTitle || import_path8.default.basename(projectDir),
    name: "project",
    thumb: { url: "", key: "" },
  ...
  try {
    ...
    if (res.data?.code === "SUCC") {
      ...
      const newDirPath = import_path8.default.join(import_path8.default.dirname(projectDir), newFolderName);
      import_fs8.default.renameSync(projectDir, newDirPath);
      ...
      try {
        ...
        if (session && session.member && session.member.project) {
          ...
          if (!session.member.project.owner.includes(newId)) {
            ...
            import_fs8.default.writeFileSync(import_path8.default.join(process.cwd(), SESSION_FILE2), JSON.stringify(session, null, 2));
...
function reassignAllPageIds(projectDir) {
  ...
  for (const oldPageFolder of pageDirs) {
    const oldPageDir = import_path8.default.join(projectDir, oldPageFolder);
    let newPageId;
    do {
      newPageId = utilIdStamp();
    ...
    const newPageFolder = `${pageTitle} (${newPageId})`;
    const newPageDir = import_path8.default.join(projectDir, newPageFolder);
    import_fs8.default.renameSync(oldPageDir, newPageDir);
    const metaPath = import_path8.default.join(newPageDir, PAGE_META_FILE2);
    if (import_fs8.default.existsSync(metaPath)) {
      const meta = readJsonSyncSafe(metaPath);
...
async function resolveDuplicateProjectIds(root) {
  const session = loadSession2();
  const cookiesArray = session.cookies;
  const projectDirs = import_fs8.default.readdirSync(root).filter((name) => import_fs8.default.existsSync(import_path8.default.join(root, name, PROJECT_META_FILE2)));
  const idToFolders = {};
  const foldersWithoutId = [];
  for (const dirName of projectDirs) {
    let dirPath = import_path8.default.join(root, dirName);
    let folderId = extractProjectIdFromFolderName3(dirName);
    const hiddenId = readHiddenProjectId2(dirPath);
    let metaId = null;
    let metaPath = import_path8.default.join(dirPath, PROJECT_META_FILE2);
    if (import_fs8.default.existsSync(metaPath)) {
      try {
        const meta = readJsonSyncSafe(metaPath);
    ...
    if (syncId) {
      if (folderId !== syncId) {
        ...
        const newDirPath = import_path8.default.join(root, newDirName);
        import_fs8.default.renameSync(dirPath, newDirPath);
        dirPath = newDirPath;
        metaPath = import_path8.default.join(dirPath, PROJECT_META_FILE2);
      }
      if (hiddenId !== syncId) {
    ...
    idToFolders[projectId].push({ dirName: import_path8.default.basename(dirPath), dirPath });
  }
  ...
  for (const [projectId, folders] of Object.entries(idToFolders)) {
    if (folders.length > 1) {
      ...
      for (let i = 1; i < folderInfos.length; i++) {
        ...
        const metaPath = import_path8.default.join(oldDirPath, PROJECT_META_FILE2);
        if (import_fs8.default.existsSync(metaPath)) {
          try {
            const meta = readJsonSyncSafe(metaPath);
        ...
        try {
          ...
          console.warn(
            `\u26A0\uFE0F \uD504\uB85C\uC81D\uD2B8 ID \uC911\uBCF5 \uAC10\uC9C0: "${oldDirName}" \u2192 "${import_path8.default.basename(
              newDirPath
            )}" (\uC11C\uBC84\uC5D0 \uC0C8 \uD504\uB85C\uC81D\uD2B8 \uC0DD\uC131)`
          );
  ...
  for (const { dirName, dirPath } of foldersWithoutId) {
    ...
    const metaPath = import_path8.default.join(dirPath, PROJECT_META_FILE2);
    if (import_fs8.default.existsSync(metaPath)) {
      try {
        const meta = readJsonSyncSafe(metaPath);
        if (meta?.projectTitle) projectTitle = meta.projectTitle;
    ...
    try {
      ...
      console.warn(
        `\u26A0\uFE0F \uD504\uB85C\uC81D\uD2B8 ID \uC5C6\uC74C: "${dirName}" \u2192 "${import_path8.default.basename(
          newDirPath
        )}" (\uC11C\uBC84\uC5D0 \uC0C8 \uD504\uB85C\uC81D\uD2B8 \uC0DD\uC131)`
      );
      newlyGeneratedIds.add(newId);
...
async function saveHiddenPageId(pageDir, pageId) {
  try {
    const hiddenPath = import_path8.default.join(pageDir, ".page_id");
    if (import_fs8.default.existsSync(hiddenPath)) {
      try {
        import_fs8.default.unlinkSync(hiddenPath);
      } catch (e) {
...
async function confirmMoveServerOnlyProjectsToTrash(serverOnlyProjects) {
  console.log("");
  logWarning2(
    "\uC804\uCCB4 push \uC2DC \uB85C\uCEEC \uC6CC\uD06C\uC2A4\uD398\uC774\uC2A4\uC5D0 \uC5C6\uB294 \uB0B4 \uC18C\uC720(owner) \uD504\uB85C\uC81D\uD2B8\uB294 \uC11C\uBC84\uC5D0\uC11C \uD504\uB85C\uC81D\uD2B8 \uD734\uC9C0\uD1B5\uC73C\uB85C \uB4E4\uC5B4\uAC11\uB2C8\uB2E4.",
    "\u26A0\uFE0F"
  );
  console.log("");
  console.log(import_chalk4.default.bold("\uC544\uB798 \uD504\uB85C\uC81D\uD2B8\uAC00 \uB85C\uCEEC\uC5D0 \uC5C6\uC5B4 \uC774\uB3D9 \uB300\uC0C1\uC785\uB2C8\uB2E4."));
  console.log(
    import_chalk4.default.dim("\uBCF5\uAD6C: npm run project-trash:restore \uB610\uB294 \uC5D0\uB514\uD130\uC758 \uD504\uB85C\uC81D\uD2B8 \uD734\uC9C0\uD1B5 \uBA54\uB274")
  );
  console.log("");
  serverOnlyProjects.forEach((project, idx) => {
    console.log(
      import_chalk4.default.yellow(
        `   ${idx + 1}. ${project.title || project.id} (${project.id})`
      )
    );
  });
  console.log("");
  return new Promise((resolve) => {
    ...
    rl.question(
      import_chalk4.default.cyan.bold("\uC704 \uD504\uB85C\uC81D\uD2B8\uB97C \uD734\uC9C0\uD1B5\uC73C\uB85C \uC774\uB3D9\uD560\uAE4C\uC694? (Y/N): "),
      (answer) => {
        rl.close();
...
async function moveServerOnlyProjectsToTrash(localProjectDirs, cookiesArray) {
  try {
    ...
    for (const projectName of localProjectDirs) {
      const projectId = extractProjectIdFromFolderName3(projectName);
      if (projectId) {
    ...
    if (shouldOfferProjectChoiceMenu()) {
      const ok = await confirmMoveServerOnlyProjectsToTrash(serverOnlyProjects);
      if (!ok) {
        logInfo2(
          "\uD734\uC9C0\uD1B5 \uC774\uB3D9\uC740 \uCDE8\uC18C\uB418\uC5C8\uC2B5\uB2C8\uB2E4. \uB85C\uCEEC\uC5D0 \uC788\uB294 \uD504\uB85C\uC81D\uD2B8\uB9CC push\uB97C \uACC4\uC18D\uD569\uB2C8\uB2E4.",
          "\u{1F4CE}"
        );
        return;
      }
      ...
      console.log(
        import_chalk4.default.yellow(
          `\u{1F4A1} \uB85C\uCEEC\uC5D0 \uC5C6\uB294 \uB0B4 \uC18C\uC720 \uD504\uB85C\uC81D\uD2B8 ${serverOnlyProjects.length}\uAC1C\uAC00 \uD734\uC9C0\uD1B5\uC73C\uB85C \uC774\uB3D9\uB429\uB2C8\uB2E4 (\uBE44\uB300\uD654\uD615\xB7TTY \uC544\uB2D8, \uD655\uC778 \uC0DD\uB7B5).`
        )
      );
    }
...
function removeBlockIdsFromWorkspace(root) {
  try {
    ...
    for (const projectName of projectDirs) {
      const projectDir = import_path8.default.join(root, projectName);
      const pageDirs = import_fs8.default.readdirSync(projectDir).filter((name) => import_fs8.default.existsSync(import_path8.default.join(projectDir, name, PAGE_META_FILE2)));
      for (const pageFolderName of pageDirs) {
        const pageDir = import_path8.default.join(projectDir, pageFolderName);
        const sectionTypes = ["header", "content", "footer", "sideleft", "sideright", "bodyStart", "bodyEnd", "stickyleft", "stickyright"];
        for (const section of sectionTypes) {
          const sectionPath = import_path8.default.join(pageDir, section);
          if (!import_fs8.default.existsSync(sectionPath)) continue;
          const blockDirs = import_fs8.default.readdirSync(sectionPath).filter((name) => {
            const blockPath = import_path8.default.join(sectionPath, name);
            return import_fs8.default.lstatSync(blockPath).isDirectory() && import_fs8.default.existsSync(import_path8.default.join(blockPath, BLOCK_META_FILE2));
          });
          for (const blockFolderName of blockDirs) {
            const blockDir = import_path8.default.join(sectionPath, blockFolderName);
            const blockMetaPath = import_path8.default.join(blockDir, BLOCK_META_FILE2);
            try {
              const meta = readJsonSyncSafe(blockMetaPath);
              if (meta && meta.id) {
                delete meta.id;
                import_fs8.default.writeFileSync(blockMetaPath, JSON.stringify(meta, null, 2));
              }
            } catch (e) {
            }
          }
...
async function updateFileReferences(projectDir, oldFileName, newFileName, fileType) {
  try {
    ...
    const scanAndUpdate = (dir) => {
      ...
      for (const item of items) {
        const itemPath = import_path8.default.join(dir, item);
        const stat = import_fs8.default.statSync(itemPath);
        if (stat.isDirectory()) {
          scanAndUpdate(itemPath);
        } else if (stat.isFile()) {
          const ext = import_path8.default.extname(item).toLowerCase();
          if (fileTypes.includes(ext)) {
            try {
              let content = import_fs8.default.readFileSync(itemPath, "utf8");
              let hasChanges = false;
              replacePatterns.forEach((pattern) => {
                const patterns = [
                  // 상대 경로 패턴 (../resources/images/파일명)
                  new RegExp(`(\\.\\.\\/)*resources\\/(?:images\\/|icons\\/|video\\/)?${escapeRegExp(pattern)}`, "g"),
                  // 절대 경로 패턴 (/resources/images/파일명)
                  new RegExp(`\\/resources\\/(?:images\\/|icons\\/|video\\/)?${escapeRegExp(pattern)}`, "g"),
                  // 파일명만 있는 패턴 (파일명)
              ...
              if (hasChanges) {
                import_fs8.default.writeFileSync(itemPath, content, "utf8");
                updatedFiles++;
                logInfo2(`\uD30C\uC77C \uCC38\uC870 \uC5C5\uB370\uC774\uD2B8: ${import_path8.default.relative(projectDir, itemPath)}`, "\u{1F4DD}");
              }
            } catch (error) {
              logWarning2(`\uD30C\uC77C \uC77D\uAE30/\uC4F0\uAE30 \uC2E4\uD328: ${import_path8.default.relative(projectDir, itemPath)} - ${error.message}`, "\u26A0\uFE0F");
            }
          }
        }
      }
